...
export function Device({
    ...
    const bezelShade = `inset 0 0 15px ${darkMode ? "#000" : Color.toRgbString(Color.alpha(backgroundColor, 0.75))}`
    ...
    const useContainerBasedScale = RenderTarget.current() === RenderTarget.preview
    ...
    const { transformPagePoint: originalTransformPagePoint } = useContext(MotionPluginContext)
            ...
            >
                ...
                >
                    <MotionPlugins transformPagePoint={transformPagePoint}>
                        {resizedChild ? <Navigation>{resizedChild}</Navigation> : null}
                    </MotionPlugins>
                </div>
            </div>
...
function customDeviceConfig(title: string, min: number, max: number): ControlDescription<Partial<Props>> {
    return {
